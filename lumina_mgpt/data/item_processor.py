...
class FlexARItemProcessor(MMConvItemProcessor):
    ...
    def process_image(self, image) -> Dict:
        """
        converts an image into the model’s image token sequence with explicit row breaks and dimensional hints, 
        wraps it with start/end tokens, and returns it as both inputs and labels for next-token prediction.
        """
        ...
        w_grids, h_grids = image.size[0] // self.patch_size, image.size[1] // self.patch_size  #down-sample
