        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if local_position_ids is None:
                ...
                past_key_value.key_cache[self.layer_idx] = past_keys  # 更新
    ...
    def forward(
        ...
        if past_key_value is not None:
            if local_position_ids is None:
                ...
                # past_key_value.value_cache[self.layer_idx] = past_values
                # key_states = past_keys
                # value_states = past_values   #先存

                # M2
                # past_keys = past_key_value.key_cache[self.layer_idx]
                # past_values = past_key_value.value_cache[self.layer_idx]

                # big_keys = torch.cat([past_keys, key_states], dim=2)   
                # big_values = torch.cat([past_values, value_states], dim=2)

                # past_len = past_keys.shape[2]
                # insert_len = key_states.shape[2]
                # total_len = past_len + insert_len

                # insert_idx = torch.arange(past_len, total_len, device=past_keys.device)
                # past_idx = torch.arange(past_len, device=past_keys.device)
                # order = torch.full((total_len,), -1, device=past_keys.device, dtype=torch.long)
                # local_position_ids = local_position_ids.flatten()
                # order.scatter_(0, local_position_ids, insert_idx)
                # mask = order.eq(-1)
                # order[mask] = past_idx
                
                # past_keys = big_keys.index_select(2, order)
                # past_values = big_values.index_select(2, order)
                ...
                B, H, T, D = past_keys.shape
                N = key_states.shape[2]
                total_len = T + N

                loc = local_position_ids.flatten().to(dtype=torch.long, device=past_keys.device)

                mask = torch.zeros(total_len, dtype=torch.bool, device=past_keys.device)
                mask[loc] = True                                 
                past_targets = (~mask).nonzero(as_tuple=False).squeeze(-1)  

                new_keys = torch.empty((B, H, total_len, D), device=past_keys.device,  dtype=past_keys.dtype)
                new_vals = torch.empty((B, H, total_len, D), device=past_values.device, dtype=past_values.dtype)

                new_keys.index_copy_(2, past_targets, past_keys) 
                new_vals.index_copy_(2, past_targets, past_values)

                new_keys.index_copy_(2, loc, key_states)          
                new_vals.index_copy_(2, loc, value_states)

                past_key_value.key_cache[self.layer_idx]  = new_keys
                past_key_value.value_cache[self.layer_idx] = new_vals
                key_states  = new_keys
                value_states = new_vals
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    Args:
        config: ChameleonConfigCHAMELEON_INPUTS_DOCSTRING
    """
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids) # dim=4096
    ...
    def _update_causal_mask(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask is not None:
                ...
                padding_mask = causal_mask[:, :, :, :mask_length] + attention_mask[:, None, None, :] # 默认(B, seq_len)
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False) #4096,65536
    ...
    def forward(
        ...
        # torch.cuda.synchronize()
        # start = time.time()
        outputs = self.model(        # ChameleonModel(config)
            input_ids=input_ids,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if attention_mask is not None and position_ids is None:
            ...
            position_ids = attention_mask.long().cumsum(-1) - 1 # (B, target_len)  (B, seq_len, target_len)
