...
class FlexARInferenceSolver:
    ...
    def generate(
        ...
        conversations = []
        for q, a in qas:   #round qustion-anwser
        ...
        item = {"image": images, "conversations": conversations}  # image and text

        _prompt = self.item_processor.process_item(item) # token 
        prompt = []
        ...
        generation_config = GenerationConfig(
            max_new_tokens=max_gen_len, #8192
            max_length=self.model.config.max_position_embeddings, #4096
            temperature=temperature, #1.0
            top_k=None,
            do_sample=True, #sample or greedy
            eos_token_id=[8710], #end token id
        )
    ...
    def create_image_grid(images, rows, cols):
        width, height = images[0].size  #(768,768)
