...
class PartiPromptsMultiGPUBench(Dataset):
    ...
    def __init__(
        ...
        self.df = split_dataframe_for_gpu(self.df, gpu_id, gpu_ids, node_id, node_ids) # 划分self.prompt_dict
    ...
    def check_all_prompts(self):
        ...
        for idx in range(len(self.df)):
            prompt = self.df.iloc[idx]['Prompt'] #prompt
            # prompt = self.clean_prompt(prompt)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for idx in range(len(self.df)):
            self.anno["annotations"].append({
                ...
                "image_id": self.prompt_dict[self.df.iloc[idx]['Prompt']]   # global idx
            })

    def __getitem__(self, idx):
        prompt = self.anno["annotations"][idx]
        return prompt
...
class CustomBench(Dataset): 
    def __init__(
        ...
        json_file = annFile
        print(f"Loading prompts from {json_file} for GPU {gpu_id}, Node {node_id}")
        
        with open(json_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.annotations = data["annotations"] 
        self.df = split_datalist_for_gpu(self.annotations, gpu_id, gpu_ids, node_id, node_ids)
        self.anno = {"annotations": self.df}
    def __len__(self):
        return len(self.df)
