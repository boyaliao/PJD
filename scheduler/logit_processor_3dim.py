...
class MultiModalLogitsProcessor_JACOBI(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.num_image_start_tokens == self.num_image_end_tokens:
            ...
            if new_token_num >= 2:
                ...
                tokens = input_ids[0][self.image_start_token_id_index + 3 :] #已经生成的
...
def get_double_cfg_input_ids(input_ids, neg_input_ids, pad_category):
    ...
    new_neg_input_ids[:batchsize, -input_ids.shape[1]:] = input_ids  # right-align
